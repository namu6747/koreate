...
public class MemberManagement extends AppBase{
	...
	private void deleteMember() {
		...
		ArrayList<BackUpMemberVO> deletes = dao.deleteMember();
		System.out.println("========================");
		for(BackUpMemberVO m : deletes) {
	...
	protected void join() {
		...
		MemberVO m = dao.join(new MemberVO(mName,mId,mPw));
	...
	protected void login() {
		...
		String mId = getStringData("사용자 정보를 입력해주세요.\n아이디를 입력해주세요 > ");
		String mPw = getStringData("비밀번호를 입력해 주세요 > ");
		MemberVO member = dao.selectMember(mId, mPw);
		if(member == null) {
